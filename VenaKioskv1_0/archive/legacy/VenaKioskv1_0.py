        ...
        try:
            for wid in self._client_window_ids():
                try:
                    ...
                    if prop and prop.value and prop.value[0] == pid:
                        return wid
                except Exception:
                    continue
        except Exception as exc:
            logger.debug("Szukanie okna po PID nie powiodło się: %s", exc)
        return None
        ...
        try:
            ...
            if self._log_file and not self._log_file.closed:
                try:
                    ...
                    pass
            self._log_file = open(CHROMIUM_LOG_PATH, "a", encoding="utf-8", errors="ignore")
...
class ChromiumAppWindowManager:
    ...
    def _watchdog_tick(self) -> None:
        ...
        if self.wid and not self.x11.is_valid(self.wid):
            logger.warning("WID %s (%s) nieprawidłowy — szukam ponownie.", self.wid, self.name)
            self.wid = None
            wid = self._find_window()
            if not wid:
                return
            ...
            if self.visible:
                ...
                self.x11.stow_window(wid)
            logger.info(
                "Okno %s odzyskane (WID %s, visible=%s)", self.name, wid, self.visible
            )
...
class WifiPage(QWidget):
    ...
    def _refresh(self):
        ...
        self._start_worker(job, self._on_scan_ok, self._on_scan_err)

    def _start_worker(self, fn, on_ok, on_err):
        if self._worker is not None and self._worker.isRunning():
            return
        worker = _WifiWorker(fn, self)
        self._worker = worker
    ...
    def _on_scan_ok(self, networks):
        ...
        active = next((n.ssid for n in networks if n.in_use), None)
        if active:
...
class KioskMainWindow(QWidget):
    ...
    def _service_quit_app(self):
        ...
        self.spotify.chromium.terminate()
        self.youtube.chromium.terminate()
